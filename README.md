...
首页先读取 bootstrap.json（首批 20 条和筛选项），不阻塞等待全量索引。无筛选分页每次只取 200 条浏览分块。按字母查询只下载相应压缩分片；任意关键词和全局条件筛选按需下载 gzip 全局索引，在后台线程解压和搜索。搜索名称预先标准化，详情直接使用结果已有字段，不重复下载分片。旧浏览器不支持 DecompressionStream 时退回 JSON。字母分片仍保留。索引、分片和浏览分块共享新的内容版本，避免旧缓存混用。

运行 `pnpm run test:data` 检查真实数据下的按需请求行为、gzip 解码和失败重试（须先更新数据）。

生成数据自动保留当前及最近六个旧版本，控制静态文件总量。打开超过保留期的旧页面遇到加载错误时应刷新。
